...
if ( process.env.NEW_RELIC_HOME ) {
  require("newrelic"); // APM 监控
}

var SAMPLE_STATS_INTERVAL = 60*1000; // 1 minute  统计几个
var SAMPLE_LOAD_INTERVAL = 5*60*1000; // 5 minutes 加载间隔？
var EMPTY_ROOM_LOG_TIMEOUT = 3*60*1000; // 3 minutes 清空日志
var WEBSOCKET_COMPAT = true; // 兼容
...
Logger.prototype = {
  ...
  write: function () {
    if (this.stdout) {
	  ...
	  // console.log.apply(console, [1, 2]); 与console.log([1, 2])
	  // 1 2  //  [1, 2]
    }
    if (this.file) {
      var s = [];
 ...
 * */
  ...
  var base = protocol + "//" + host;
	// 直接用浏览器访问  127.0.0.1:8080/status, 页面显示ok  证明服务成功
  if (url.pathname == '/status') {
    ...
    if (request.method == "OPTIONS") {
      // CORS preflight  --- > options 请求 预请求， 保护不支持 cors 的旧服务器
...
wsServer.on('request', function(request) {
  ...
  connection.on('message', function(message) {
	console.log(1111, JSON.stringify(message), 222222222)
    var parsed;
    try {
...
// 如果是node直接运行的， require.main === module  为true， 如果是引用的 ，false
if (require.main == module) {
  ...
  var host = ops.argv.host || process.env.HUB_SERVER_HOST || process.env.VCAP_APP_HOST ||
      process.env.HOST || '192.168.10.59' || '127.0.0.1';
